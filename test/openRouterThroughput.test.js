...
describe('OpenRouter Throughput Routing', () => {
  ...
  beforeEach(() => {
    ...
    process.env.VZCODE_AI_BASE_URL = 'https://openrouter.ai/api/v1';
  });
  ...
  test('generateAIResponse should include provider.sort=throughput parameter', async () => {
    ...
    const { generateAIResponse } = await import('../src/server/generateAIResponse.ts');
  ...
  test('handleAICopilot should include provider.sort=throughput in additionalParameters', async () => {
    ...
    process.env.VZCODE_AI_COPILOT_API_KEY = 'test-copilot-key';
    process.env.VZCODE_AI_COPILOT_BASE_URL = 'https://openrouter.ai/api/v1';
    ...
    vi.doMock('@langchain/core/output_parsers', () => ({
      StringOutputParser: vi.fn(() => ({
        invoke: vi.fn().mockResolvedValue('parsed response')
      }))
    }));

    // Import and test the handler
    const { handleAICopilot } = await import('../src/server/handleAICopilot.ts');
    const handler = handleAICopilot();

    // Mock request and response
    const mockReq = {
      body: { prefix: 'test prefix', suffix: 'test suffix' }
    };
    const mockRes = {
    ...
    expect(mockRes.send).toHaveBeenCalledWith({ text: 'parsed response' });
  });
});