...
Only include the files that need to be updated or created.`;
    ...
    const parsed = parseMarkdownFiles(problematicInput, 'bold');
    console.log('Parsed files:', Object.keys(parsed.files || {}));
    
    // This should show the problem - extra files being created
    expect(Object.keys(parsed.files || {})).toContain('getUSAtlasData.js');
    expect(Object.keys(parsed.files || {})).toContain('viz.js');
    // The problem: these extra files should NOT be created
    expect(Object.keys(parsed.files || {})).toContain('fileA.js');
    expect(Object.keys(parsed.files || {})).toContain('fileB.js');
  });
...
Only include the files that need to be updated or created.`;
    ...
    const preprocessed = problematicInput.split('## Formatting Instructions')[0].trim();
    const parsedFixed = parseMarkdownFiles(preprocessed, 'bold');
    
    console.log('=== Testing with preprocessing ===');
    console.log('Parsed files after preprocessing:', Object.keys(parsedFixed.files || {}));
    
    // After preprocessing, only the real files should be present
    expect(Object.keys(parsedFixed.files || {})).toContain('getUSAtlasData.js');
    expect(Object.keys(parsedFixed.files || {})).toContain('viz.js');
    // The fix: these extra files should NOT be created
    expect(Object.keys(parsedFixed.files || {})).not.toContain('fileA.js');
    expect(Object.keys(parsedFixed.files || {})).not.toContain('fileB.js');
    
    // Should only have 2 files
    expect(Object.keys(parsedFixed.files || {}).length).toBe(2);
  });
});