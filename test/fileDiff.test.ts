...
describe('fileDiff', () => {
  describe('generateFileDiff', () => {
    it('should generate correct diff for file changes', () => {
      ...
      const diff = generateFileDiff('file1', 'index.js', beforeContent, afterContent);
      ...
      expect(diff.lines[0].content).toBe('// Welcome to my visualization');
      expect(diff.lines[1].type).toBe('unchanged');
      expect(diff.lines[1].content).toBe('console.log("Hello, World!");');
    });

    it('should detect no changes when content is identical', () => {
      const content = 'console.log("Hello, World!");';
      
      const diff = generateFileDiff('file1', 'index.js', content, content);
    ...
    it('should handle removals correctly', () => {
      const beforeContent = '// Old comment\nconsole.log("Hello, World!");';
  ...
  describe('generateFilesDiff', () => {
    it('should generate diffs for multiple files', () => {
      const beforeFiles: VizFiles = {
        'file1': { name: 'index.js', text: 'console.log("Hello");' },
        'file2': { name: 'style.css', text: 'body { color: red; }' },
      };

      const afterFiles: VizFiles = {
        'file1': { name: 'index.js', text: '// Updated\nconsole.log("Hello");' },
        'file2': { name: 'style.css', text: 'body { color: red; }' }, // No change
      };

      const diffs = generateFilesDiff(beforeFiles, afterFiles);
      ...
      expect(diffs['file1'].lines[0].content).toBe('// Updated');
    });
    ...
    it('should handle new files', () => {
      ...
      const afterFiles: VizFiles = {
        'file1': { name: 'index.js', text: 'console.log("Hello");' },
        'file2': { name: 'new.js', text: 'console.log("New file");' },
      };
      ...
      expect(diffs['file2'].afterContent).toBe('console.log("New file");');
    });
  });
