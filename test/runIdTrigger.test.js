...
describe('runId triggering functionality', () => {
  ...
  it('should determine correct hot reloading settings', () => {
    ...
    const getHotReloadingSetting = (isInteracting, runIdChanged) => {
    ...
    expect(getHotReloadingSetting(false, false)).toBe(false);
  ...
  it('should determine when to trigger runs', () => {
    ...
    const shouldTriggerRun = (isFirstRun, isInteracting, runIdChanged) => {
    ...
    expect(shouldTriggerRun(false, false, false)).toBe(false);
  });
});