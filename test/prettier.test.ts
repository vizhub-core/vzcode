import { formatFile, formatFiles } from '../src/server/prettier.js';
...
describe('Server-side Prettier Integration', () => {
  it('should format JavaScript code correctly', async () => {
    ...
    const result = await formatFile(unformattedJS, 'test.js');
    
    expect(result).toBeTruthy();
    expect(result).toContain('const x = 1;');
    expect(result).toMatch(/let y = \{\s*a: 1,\s*b: 2,\s*c: 3,?\s*\};/);
  });
  ...
  it('should format TypeScript code correctly', async () => {
    ...
    const result = await formatFile(unformattedTS, 'test.ts');
  ...
  it('should format CSS code correctly', async () => {
    ...
    const result = await formatFile(unformattedCSS, 'test.css');
  ...
  it('should return null for unsupported file types', async () => {
    const result = await formatFile('some content', 'test.txt');
