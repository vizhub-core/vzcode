...
vi.mock('llm-code-format', () => ({
  formatMarkdownFiles: vi.fn((files) => `**file1.js**
...
vi.mock('editcodewithai', () => ({
  FORMAT_INSTRUCTIONS: { whole: '## Formatting Instructions\n\nMock instructions' },
  mergeFileChanges: vi.fn((existing, parsed) => ({ ...existing, ...parsed }))
}));
...
describe('AI Copy Paste Integration', () => {
  test('handlePasteForAI should ignore content after "## Formatting Instructions"', async () => {
    const mockFiles = {
      'existing.js': { name: 'existing.js', text: 'console.log("existing");' }
    };
...
\`\`\``;
    ...
    navigator.clipboard.readText.mockResolvedValue(clipboardContent);
    ...
    expect(setPasteButtonText).toHaveBeenCalledWith('Pasted!');
  });
