...
export const DiffView: React.FC<DiffViewProps> = ({ diffData }) => {
  const fileDiffs = Object.values(diffData).filter(diff => diff.hasChanges);
  ...
  const totalAdditions = fileDiffs.reduce(
    (sum, diff) => sum + diff.lines.filter(line => line.type === 'added').length,
    0
  );
  
  const totalDeletions = fileDiffs.reduce(
    (sum, diff) => sum + diff.lines.filter(line => line.type === 'removed').length,
    0
  );
