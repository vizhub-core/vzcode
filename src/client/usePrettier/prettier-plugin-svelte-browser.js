...
function snipScriptAndStyleTagContent(source) {
  ...
  function getMatchIndexes(tagName) {
    ...
    while ((match = regex.exec(source)) != null) {
      if (
        source.slice(match.index, match.index + 4) !==
        '<!--'
      ) {
  ...
  function snipTagContent(
    ...
    const newSource = _source.replace(
      ...
      (match, attributes, content, index) => {
        ...
        newStyleMatchSpans = adjustSpans(
          styleMatchSpans,
          newStyleMatchSpans,
        );
        function adjustSpans(oldSpans, newSpans) {
          return oldSpans.map((oldSpan, idx) => {
            ...
            if (oldSpan[0] > index) {
              ...
              return [
                newSpan[0] - lengthDiff,
                newSpan[1] - lengthDiff,
              ];
    ...
    function withinOtherSpan(idx) {
      return otherSpans.some(
        (otherSpan) =>
          idx > otherSpan[0] && idx < otherSpan[1],
      );
    }
  }
...
function parseSortOrder(
  ...
  if (!order.includes('options')) {
    throw new Error(
      'svelteSortOrder is missing option `options`',
    );
  }
  return order;
}
...
function getAttributeLine(node, options) {
  ...
  const attributes = node.attributes.filter(
    (attribute) =>
      attribute.name !== snippedTagContentAttribute,
  );
  return options.singleAttributePerLine &&
    (attributes.length > 1 ||
      (attributes.length && hasThisBinding))
    ? hardline
    : line;
}
function printWithPrependedAttributeLine(
  node,
  options,
  print,
) {
...
function isHardline(docToCheck) {
  return (
    ...
    deepEqual(docToCheck, doc.builders.hardline)
  );
}
/**
...
function getLeadingComment(path) {
  ...
  let prev = siblings.find(
    (child) => child.end === node.start,
  );
  while (prev) {
    if (
      prev.type === 'Comment' &&
 ...
 */
function doesEmbedStartAfterNode(
  node,
  path,
  siblings = getSiblings(path),
) {
...
function isNodeTopLevelHTML(node, path) {
  ...
  return (
    !!root.html &&
    !!root.html.children &&
    root.html.children.includes(node)
  );
}
...
function printRaw(
  ...
  let raw = originalText.substring(
    firstChild.start,
    lastChild.end,
  );
...
function getLangAttribute(node) {
  const value =
    getAttributeTextValue('lang', node) ||
    getAttributeTextValue('type', node);
  if (value != null) {
...
function isJSON(node) {
  ...
  return (
    lang.endsWith('json') || lang.endsWith('importmap')
  );
}
...
function isTextNodeStartingWithLinebreak(
  node,
  nrLines = 1,
) {
  return (
    node.type === 'Text' &&
    startsWithLinebreak(getUnencodedText(node), nrLines)
  );
}
...
function isTextNodeEndingWithLinebreak(node, nrLines = 1) {
  return (
    node.type === 'Text' &&
    endsWithLinebreak(getUnencodedText(node), nrLines)
  );
}
...
function trimChildren(children, path) {
  ...
  lastNonEmptyNode =
    lastNonEmptyNode === -1 ? 0 : lastNonEmptyNode;
 ...
 */
function checkWhitespaceAtStartOfSvelteBlock(
  node,
  options,
) {
  ...
  if (
    parentOpeningEnd > 0 &&
    firstChild.start > parentOpeningEnd + 1
  ) {
    ...
    if (textBetween.trim() === '') {
      return startsWithLinebreak(textBetween)
        ? 'line'
        : 'space';
    }
  }
...
function checkWhitespaceAtEndOfSvelteBlock(node, options) {
  ...
  const parentClosingStart = options.originalText.indexOf(
    '{',
    lastChild.end,
  );
  if (
    parentClosingStart > 0 &&
    lastChild.end < parentClosingStart
  ) {
    const textBetween = options.originalText.substring(
      lastChild.end,
      parentClosingStart,
    ...
    if (textBetween.trim() === '') {
      return endsWithLinebreak(textBetween)
        ? 'line'
        : 'space';
    }
  }
...
function assignCommentsToNodes(ast) {
  if (ast.module) {
    ast.module.comments = removeAndGetLeadingComments(
      ast,
      ast.module,
    );
  }
  if (ast.instance) {
    ast.instance.comments = removeAndGetLeadingComments(
      ast,
      ast.instance,
    );
  }
  if (ast.css) {
    ast.css.comments = removeAndGetLeadingComments(
      ast,
      ast.css,
    );
  }
...
function print(path, options, print) {
  ...
  if (
    (ignoreNext ||
      (ignoreRange && !isIgnoreEndDirective(node))) &&
  ...
  switch (node.type) {
    case 'Fragment':
      ...
      if (
        children.length === 0 ||
        children.every(isEmptyTextNode)
      ) {
        return '';
      }
    ...
    case 'Title': {
      ...
      const didSelfClose =
        options.originalText[node.end - 2] === '/';
      ...
      if (isEmpty) {
        ...
        body = () =>
          printPre(node, options.originalText, path, print);
      } else if (!isSupportedLanguage) {
        body = () =>
          printRaw(node, options.originalText, true);
      } else if (
        isInlineElement(path, options, node) &&
        !isPreTagContent(path)
      ) {
    ...
    case 'Attribute': {
      if (isOrCanBeConvertedToShorthand(node)) {
        ...
        const quotes =
          !isLoneMustacheTag(node.value) ||
          ((_a = options.svelteStrictMode) !== null &&
          _a !== void 0
            ? _a
            : false);
    ...
    case 'ElseBlock': {
      ...
      if (
        ...
        const def = [
          ...
          path.map(
            (ifPath) =>
              printJS(ifPath, print, 'expression'),
            'children',
          )[0],
          '}',
          path.map(
    ...
    case 'AwaitBlock': {
      ...
      const hasThenBlock = node.then.children.some(
        (n) => !isEmptyTextNode(n),
      );
    ...
    case 'StyleDirective':
      ...
      if (
        isOrCanBeConvertedToShorthand(node) ||
        node.value === true
      ) {
        if (options.svelteAllowShorthand) {
          return [...prefix];
        ...
        const quotes =
          !isLoneMustacheTag(node.value) ||
          ((_b = options.svelteStrictMode) !== null &&
          _b !== void 0
            ? _b
            : false);
        const attrNodeValue = printAttributeNodeValue(
...
function printTopLevelParts(n, options, path, print) {
  if (options.svelteSortOrder === 'none') {
    ...
    if (
      ...
      !hasPragma(options.originalText)
    ) {
  ...
  const docs = flatten(
    parseSortOrder(options.svelteSortOrder).map(
      (p) => parts[p],
    ),
  );
...
function printSvelteBlockChildren(path, print, options) {
  ...
  const whitespaceAtStartOfBlock =
    checkWhitespaceAtStartOfSvelteBlock(node, options);
  const whitespaceAtEndOfBlock =
    checkWhitespaceAtEndOfSvelteBlock(node, options);
  const startline =
    whitespaceAtStartOfBlock === 'none'
      ? ''
      : whitespaceAtEndOfBlock === 'line' ||
          whitespaceAtStartOfBlock === 'line'
        ? hardline
        : line;
  const endline =
  ...
  return [
    indent([
      startline,
      group(printChildren(path, print, options)),
    ]),
    endline,
  ];
}
...
function prepareChildren(children, path, print, options) {
  ...
  for (let idx = 0; idx < children.length; idx++) {
    ...
    if (options.svelteSortOrder !== 'none') {
      if (isCommentFollowedByOptions(currentChild, idx)) {
        ...
        idx +=
          nextChild && isEmptyTextNode(nextChild) ? 1 : 0;
        continue;
      }
  ...
  for (
    let idx = 0;
    idx < childrenWithoutOptions.length;
    idx++
  ) {
...
function _expandNode(node, parent) {
  ...
  switch (node.type) {
    ...
    case 'ArrayPattern':
      return (
        ' [' +
        node.elements.map(_expandNode).join(',').slice(1) +
        ']'
      );
    case 'AssignmentPattern':
      return (
        _expandNode(node.left) +
        ' =' +
        _expandNode(node.right)
      );
    case 'Identifier':
      return ' ' + node.name;
    case 'Literal':
    ...
    case 'ObjectPattern':
      return (
        ' {' +
        node.properties.map(_expandNode).join(',') +
        ' }'
      );
    case 'Property':
      if (
...
function extractAttributes(html) {
  const [, attributesString] = html.match(
    extractAttributesRegex,
  );
  const attrs = [];
  let match;
  while ((match = attributeRegex.exec(attributesString))) {
    const [all, name, quotes, valueQuoted, valueUnquoted] =
      match;
...
function embed(path, _options) {
  ...
  if (isASTNode(node)) {
    ...
    if (node.module) {
      ...
      node.module.attributes = extractAttributes(
        getText(node.module, options),
      );
    }
    if (node.instance) {
  ...
  switch (parent.type) {
      ...
      if (node === parent.expression) {
        parent.expression.end =
          options.originalText.indexOf(
            ...
            (_e =
              (_b =
                (_a = parent.context) === null ||
                _a === void 0
                  ? void 0
                  : _a.end) !== null && _b !== void 0
                ? _b
                : (_d =
                      (_c = parent.parameters) === null ||
                      _c === void 0
                        ? void 0
                        : _c[
    ...
    case 'Element':
      printJS$1(
        parent,
        (_f = options.svelteStrictMode) !== null &&
          _f !== void 0
          ? _f
          : false,
        false,
      ...
      if (node === parent.expression) {
        parent.expression.end =
          options.originalText.indexOf(
            ')',
            (_h =
              (_g = parent.argument) === null ||
              _g === void 0
                ? void 0
                : _g.end) !== null && _h !== void 0
              ? _h
              : parent.expression.end,
          ) + 1;
      ...
      __awaiter(this, void 0, void 0, function* () {
        try {
          const embeddedOptions = {
            ...
            singleQuote: node.forceSingleQuote
              ? true
              : options.singleQuote,
          };
  ...
  return __awaiter(this, void 0, void 0, function* () {
    try {
      ...
      const indentIfDesired = (doc) =>
        options.svelteIndentScriptAndStyle
          ? indent$1(doc)
          : doc;
      trimRight([body], isLine);
      return [
        indentIfDesired([hardline$1, body]),
        hardline$1,
      ];
    } catch (error) {
...
function embedTag(
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (isTopLevel && options.svelteSortOrder !== 'none') {
      ...
      return isTopLevel && comments.length
        ? [...comments, result]
        : result;
    }
  });
...
const parsers = {
  ...
  svelteExpressionParser: Object.assign(
    Object.assign({}, babelParser),
    {
      parse: (text, options) => {
